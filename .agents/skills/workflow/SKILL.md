...
- **`workflow_execute`** — submit a Lua script (inline `script` or `path`),
  optionally with `args`, `concurrency` (1–64), and an absolute existing
  `working_dir`. `working_dir` becomes the default directory inherited by all
  agents in a fresh run; it cannot be changed when using `resume_from_id`.
  Returns `{ run_id, status }`. Also supports `resume_from_id` to resume a
  checkpointed run.
